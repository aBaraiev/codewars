...
// so that each column, each row, and each of the nine 3x3 sub-grids (also known as blocks) contain
    ...
    for (let m = 0; m < 3; m++) {
        ...
        for (let j = 0; j < 3; j++) {
            ...
            for (let i = row; i < row + 3; i++) {
                block.push(...board[i].slice(k, k + 3))
            }
            if (!validateItem(block))
                return false;
            k += 3;
        }
        row += 3;
...
function validateItem(item) {
    return [...item].sort((a, b) => a - b).join('') === '123456789';
}